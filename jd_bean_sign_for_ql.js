...
增加金融签到，抓包地址： https://ms.jr.jd.com/gw/generic/hy/h5/m/appSign
输出变量：
export JD_jrBodyArr='[{"pin":"你的pt_pin","body":"你抓到的金融body"},{"pin":"你的pt_pin2","body":"你抓到的金融body2"},...]'
...
!(async() => {
  ...
  //金融签到body
  if (typeof process.env.JD_jrBodyArr !== "undefined") {
  ...
  for (let i =0; i < cookiesArr.length; i++) {
    ...
    if (cookie) {
      ...
      $.index = i + 1;
      $.nickName = '';
      $.pin = cookie.match(/pt_pin=(\S*);/)[1]
	  await TotalBean();
      if (Array.isArray(jrBodyArr) && jrBodyArr.length !== 0) {
		  for (let a of jrBodyArr) {
			  if (a.pin === $.pin) {
				  jrBody = a.body
				  $.newCK = '`'+JSON.stringify([{"cookie": cookie, "jrBody": jrBody}])+'`';
			  } else {
				  $.newCK = '`'+JSON.stringify([{"cookie": cookie}])+'`'
			  }
		  }
	  }
...
async function changeFile (content) {
  console.log(`开始替换变量`)
  let newContent = content.replace(/var OtherKey = `.*`/, `var OtherKey = ${$.newCK}`); //增加金融签到的body
